  ...
  return (
    <div className="sources-container">
      <button onClick={() => setIsOpen(!isOpen)} className="sources-button">
        ...
        {isOpen ? 'Hide sources' : `${chunks.length} sources`}
      </button>
      {isOpen && (
        <div className="sources-content">
          {chunks.map((chunk, index) => (
            <div key={index} className="source-chunk">
              ...
              <p>{typeof chunk === 'object' ? `Page ${chunk.page} — ${chunk.source}` : chunk}</p>
            </div>
          ))}
        </div>
      )}
...
function TypingIndicator() {
  return (
    <div className="message-container ai">
      <div className="sender-label ai-label">Jawab AI</div>
...
function ChatWindow({ activeMode, documentId }) {
  const [messages, setMessages] = useState([
    { sender: 'ai', text: '👋 Hey! I\'ve read your document. Ask me anything — I\'ll pull the answers straight from it.' }
  ]);
  const [input, setInput] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const sessionIdRef = useRef(null);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        sender: 'ai',
        text: '⚠️ Couldn\'t reach the server. Try again in a moment.'
      }]);
    } finally {
      setIsLoading(false);
      inputRef.current?.focus();
  ...
  const handleNewChat = async () => {
    if (sessionIdRef.current) {
      try {
        await axios.delete(`${import.meta.env.VITE_BASE_URL}/session/${sessionIdRef.current}`);
      } catch (e) { /* silent */ }
      sessionIdRef.current = null;
    }
    setMessages([{
      sender: 'ai',
      text: '👋 Hey! I\'ve read your document. Ask me anything — I\'ll pull the answers straight from it.'
    }]);
  };
  ...
  return (
    <div className="chat-window">
      ...
      {/* Input area */}
      <div className="input-area">
        <button onClick={handleNewChat} className="new-chat-btn" title="Start fresh">
          + New
        </button>
