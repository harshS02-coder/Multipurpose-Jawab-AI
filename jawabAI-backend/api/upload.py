...
# removing extra latency by removing cloudinary
...
def upload_document(
    ...
    # Step 1: Read file content once — reuse for everything
    file_content = file.file.read()

    # Step 2: Generate hash for cache check
    file_hash = cache_helper.get_file_hash(file_content)
    print(f"📝 File hash: {file_hash[:16]}...")

    # Step 3: Check if already processed for this use_case
    cached_document_id = cache_helper.get_cached_document_id(file_hash, use_case)
    ...
    # Step 4: New PDF — generate document_id
    document_id = str(uuid.uuid4())
    print(f"🆕 New document - processing: {document_id}")

    # Step 5: Cache the hash -> document_id mapping
    cache_helper.cache_pdf_mapping(file_hash, document_id, use_case)

    # Step 6: Start background task — pass raw bytes directly, 
    # Cloudinary upload also happens inside as backup
    background_tasks.add_task(
        ingest_pipeline,
        file_content,        # raw bytes — no Cloudinary round-trip
        use_case,
        document_id,
        file.filename        # pass filename for Cloudinary folder naming
    )
