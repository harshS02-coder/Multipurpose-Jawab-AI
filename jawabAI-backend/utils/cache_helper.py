...
class CacheHelper:
    ...
    def append_to_history(self, session_id: str, role: str, content: str, ttl: int = 7200):
        """Add one turn to session history. Keeps last 8 turns. TTL: 2 hours."""
        key = self.get_session_key(session_id)
        raw = self.redis.get(key)
        history = json.loads(raw) if raw else []
        history.append({"role": role, "content": content})
        history = history[-8:]  # keep last 8 turns (4 Q&A pairs)
    ...
    def clear_history(self, session_id: str):
        """Wipe history — e.g. when user clicks 'New chat'."""
        self.redis.delete(self.get_session_key(session_id))
